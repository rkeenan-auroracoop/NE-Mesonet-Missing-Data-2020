...
# formatting in the Dictionary is off, because linter keeps screwing up the format on save. Code still functions as it should.
...
# this function checks the LOC_NAME against keys in the dictionary, and it provides a value if the confidence is over 80. keys are the names of the mesonet sites  from the csv file that UNL gave us and the values are the names of the mesonet sites that we currently scrape from in dbo.gdp_mesonet_scrape.
